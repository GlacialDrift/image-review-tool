...
def assign_batch(con, user: str, n: int, qc_rate: float = 0.10):
    ...
    with con:
        ...
        qc_rows = con.execute("""
            ...
            RETURNING review_id;
        """, (user, target_qc, user, batch_id)).fetchall()
        ...
        non_rows = con.execute("""
            ...
            WHERE review_id IN (SELECT review_id FROM pool)
            RETURNING review_id;
        """, (target_qc - got_qc, user, target_non + max(0, target_qc - got_qc), user, batch_id)).fetchall()
...
def record_decision(con, review_id: int, user: str, batch_id: str, result: str, standard_version: str):
    with con:
        con.execute("""
            UPDATE reviews
